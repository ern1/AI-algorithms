...
def result(board, playerTurn, move):
    ...
    player_store = playerTurn * 7 - 1           # index for the players store
    ...
    if end_index in range(player_store - 6, player_store):
        # If it was dropped in an empty hole if there is 1 piece in it now
        if new_board[end_index] == 1:
            # Captue last piece and any pieces in hole directly opposite
...
def minimax_decision(board, playerTurn, max_depth=3):
    actions = [(minimax(*result(board, playerTurn, a), max_depth - 1), a) for a in get_possible_moves(board, playerTurn)]
    return str(
        max(actions, key=lambda x: x[0])[1] + 1 if playerTurn == 1 else
